...
(function(){
  ...
  btn.addEventListener('click', () => {
    ...
    localStorage.setItem(
      'filtersCollapsed',
      root.classList.contains('filters-collapsed') ? '1' : '0'
    );
...
function init(){
  ...
  (function initMaterialStatusDropdown(){
    ...
    btn.addEventListener('click', e => {
      ...
      const maxDDW = Math.min(dropdown.offsetWidth, vw - 16);
      dropdown.style.maxWidth = maxDDW + 'px';
      let left = btnRect.right - maxDDW;
      if(left < 8) left = 8;
      if(left + maxDDW > vw - 8) left = vw - maxDDW - 8;

      const maxDDH = Math.min(dropdown.offsetHeight, vh - 16);
      dropdown.style.maxHeight = maxDDH + 'px';
      let top = btnRect.bottom + gap;
      if(top + maxDDH > vh - 8){
        top = btnRect.top - maxDDH - gap;
      }
      if(top < 8) top = 8;
    ...
    document.addEventListener('click', e => {
      if(!document.getElementById('materialStatusDropdownWrap')?.contains(e.target)){
...
function showSearchMiniDetails(foundList){
  ...
  if(miniBody){
    miniBody.innerHTML = `
      <div class="mini-row"><div class="mini-k">ID</div><div class="mini-v mono">${esc(id)}</div></div>
      <div class="mini-row"><div class="mini-k">Customer</div><div class="mini-v">${esc(cust)}</div></div>
      <div class="mini-row"><div class="mini-k">Customer City</div><div class="mini-v">${esc(city || '—')}</div></div>
      <div class="mini-row"><div class="mini-k">Bin No</div><div class="mini-v mono">${esc(bin)}</div></div>
      <div class="mini-row"><div class="mini-k">Type</div><div class="mini-v">${esc(type || '—')}</div></div>
      <div class="mini-row"><div class="mini-k">From Top</div><div class="mini-v mono">${esc(pos.top)}</div></div>
      <div class="mini-row"><div class="mini-k">From Bottom</div><div class="mini-v mono">${esc(pos.bottom)}</div></div>
    `;
  }
  ...
  const html = bins.map((b, idx)=>{
    ...
    return `
      <div class="ml-bin" data-ml-bin="${esc(b.bin)}">
        <div class="ml-hdr">
          <div>
            <strong>${esc(b.bin)}</strong>
            <div class="meta">
              <b>${b.customerCount}</b> customers • <b>${b.itemCount}</b> items<br>
              ${customersUI}
            </div>
          </div>
          <div class="ml-actions">
            <button class="primary" data-ml-act="focus" data-bin="${esc(b.bin)}">Focus Bin</button>
            <button data-ml-act="openbin" data-bin="${esc(b.bin)}">Open Bin Details</button>
...
(function () {
  ...
  document.addEventListener('keydown', (ev) => {
    ...
    if ((ev.key === 'Enter' || ev.key === ' ') && t && (t.classList?.contains('__bin_clickable') || t.dataset?.bin)) {
      ...
      if (t.closest && (t.closest('#search-details') || t.closest('#mixed-lot-panel'))) return; 
...
(function(){
  ...
  function render(units){
    ...
    filtered.forEach(u=>{
      ...
      title.innerHTML = `
        <div>
          <div>${u.customer || 'Unknown Customer'}</div>
          <div class="sub">${u.city || 'Unknown City'} • <span class="pill">${u.target_mode || u.dispatch_mode || 'Truck'}</span></div>
        </div>
      ...
      sub.innerHTML = `Status: <b>Ready</b>${suggestions ? `<br><span class="pill" style="background:#fff7ed;color:#c2410c">${suggestions} moves suggested</span>` : ``}`;
      ...
      if((u.move_suggestions || []).length){
        ...
        movesWrap.innerHTML = `<div style="margin-top:10px; font-weight:700; color:var(--text-primary);">Suggested Moves</div><table class="ds-table" style="margin-top:6px;"><thead><tr><th>ID</th><th>From</th><th>To</th><th>Reason</th></tr></thead><tbody>${mvRows}</tbody></table>`;
